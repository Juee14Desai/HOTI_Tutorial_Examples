...
static int post_recv(void)
{
	...
	fi_tag = TAG_2;
	do {
		ret = fi_trecv(ep, buf_2, BUF_SIZE, NULL, fi_addr, fi_tag, 0, NULL);
		if (ret && ret != -FI_EAGAIN) {
	...
	fi_tag = TAG_1;
	do {
        	ret = fi_trecv(ep, buf_1, BUF_SIZE, NULL, fi_addr, fi_tag, 0, NULL);
        	if (ret && ret != -FI_EAGAIN) {
        	    printf("error posting recv buffer (%d\n", ret);
        	    return ret;
        	}
...
static int post_send(void)
{
	...
	fi_tag = TAG_1;
	do {
		ret = fi_tsend(ep, buf_1, strlen(msg_1), NULL, fi_addr, fi_tag, NULL);
		if (ret && ret != -FI_EAGAIN) {
	...
	fi_tag = TAG_2;
	do {
        	ret = fi_tsend(ep, buf_2, strlen(msg_2), NULL, fi_addr, fi_tag, NULL);
        	if (ret && ret != -FI_EAGAIN) {
			printf("error posting send buffer (%d)\n", ret);
        		return ret;
        	}
