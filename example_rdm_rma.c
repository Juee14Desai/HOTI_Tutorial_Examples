...
//Build with
//gcc -o example_rdm_rma example_rdm_rma.c -L<path to libfabric lib> -I<path to libfabric include> -lfabric
...
static int exchange_info(void)
{
	...
	ret = sock_send(oob_sock, &local_key_read, sizeof(local_key_read));
		...
		ret = sock_recv(oob_sock, &remote_addr_write, sizeof(remote_addr_write));
		if (ret) {
			printf("sock_recv error %d\n", ret);
			return ret;
		}
...
static int initialize(void)
{
	...
	ret = fi_mr_reg(domain, buf_read, BUF_SIZE, FI_READ | FI_REMOTE_READ, 0,
		       	MR_KEY+1, 0, &mr_read, NULL);
        if (ret) {
...
static int do_write(void)
{
	...
	do {
		ret = fi_write(ep, buf_write, strlen(msg), local_desc_write, fi_addr, remote_addr_write,
			       remote_key_write, NULL);
		if (ret && ret != -FI_EAGAIN) {
