...
//Build with
//gcc -o example_rdm_rma_cntr example_rdm_rma_cntr.c -L<path to libfabric lib> -I<path to libfabric include> -lfabric
...
struct fid_mr *mr_r = NULL;
struct fid_mr *mr_w = NULL;
struct fid_eq *eq = NULL;
void *desc, *desc_r, *desc_w;
uint64_t remote_addr, remote_addr_r, remote_addr_w, key, key_r, key_w;
char buf[BUF_SIZE];
char buf_r[BUF_SIZE];
char buf_w[BUF_SIZE];
...
static int exchange_info(void)
{
	...
	ret = sock_send(oob_sock, &key_w, sizeof(key_w));
	if (ret) {
		printf("sock_send error %d\n", ret);
		return ret;
	}

	ret = sock_recv(oob_sock, &key_w, sizeof(key_w));
	if (ret) {
		printf("sock_recv error %d\n", ret);
		return ret;
	}

	ret = sock_send(oob_sock, &key_r, sizeof(key_r));
        if (ret) {
                printf("sock_send error %d\n", ret);
                return ret;
        }

        ret = sock_recv(oob_sock, &key_r, sizeof(key_r));
        if (ret) {
                printf("sock_recv error %d\n", ret);
                return ret;
        }
	...
	if (info->domain_attr->mr_mode & FI_MR_VIRT_ADDR) {
		ret = sock_send(oob_sock, &remote_addr_w, sizeof(remote_addr_w));
		if (ret) {
			printf("sock_send error %d\n", ret);
			return ret;
		}

		ret = sock_recv(oob_sock, &remote_addr_w, sizeof(remote_addr_w));
		if (ret) {
			printf("sock_recv error %d\n", ret);
			return ret;
		}

		ret = sock_send(oob_sock, &remote_addr_r, sizeof(remote_addr_r));
                if (ret) {
                        printf("sock_send error %d\n", ret);
                        return ret;
                }

                ret = sock_recv(oob_sock, &remote_addr_r, sizeof(remote_addr_r));
                if (ret) {
                        printf("sock_recv error %d\n", ret);
                        return ret;
                }
...
static int initialize(void)
{
	...
	/* Initialize our endpoint. Endpoints are transport level communication portals which are used to
         ...
         * In this application, we requested TCP and FI_EP_RDM so we will automatically pick up RxM in order
	...
	/* Initialize our address vector. Address vectors are used to map higher level addresses, which may
         * be more natural for an application to use, into fabric specific addresses. An AV_TABLE av will
         * map these addresses to indexed addresses, starting with fi_addr 0. These addresses are used in
         * data transfer calls to specific which peer to send to/recv from.
         * Address vectors are only used for FI_EP_RDM and FI_EP_DGRAM endpoints, allowing the application
         * to avoid connection management. For FI_EP_MSG endpoints, the AV is replaced by the traditional
	...
	/* Bind the AV to the EP. The EP can only send data to a peer in its AV. */
	...
	/* Once we have all our resources initialized and ready to go, we can enable our EP in order
	...
	ret = fi_mr_reg(domain, buf_w, BUF_SIZE, FI_WRITE |
			FI_REMOTE_WRITE, 0, MR_KEY, 0, &mr_w, NULL);
	if (ret) {
		printf("fi_mr_reg error (%d)\n", ret);
		return ret;
	}

	ret = fi_mr_reg(domain, buf_r, BUF_SIZE, FI_READ | FI_REMOTE_READ, 0,
		       	MR_KEY+1, 0, &mr_r, NULL);
        if (ret) {
                printf("fi_mr_r_reg error (%d)\n", ret);
                return ret;
	}

	desc_w = fi_mr_desc(mr_w);
	key_w = fi_mr_key(mr_w);

	desc_r = fi_mr_desc(mr_r);
        key_r = fi_mr_key(mr_r);

	remote_addr_w = info->domain_attr->mr_mode & FI_MR_VIRT_ADDR ?
			(uintptr_t) buf_w : 0;

	remote_addr_r = info->domain_attr->mr_mode & FI_MR_VIRT_ADDR ?
                        (uintptr_t) buf_r : 0;

	if (info->domain_attr->mr_mode & FI_MR_ENDPOINT) {
		ret = fi_mr_bind(mr_w, &ep->fid, 0);
		if (ret) {
			printf("fi_mr_bind error (%d)\n", ret);
			return ret;
		}

		ret = fi_mr_bind(mr_r, &ep->fid, 0);
                if (ret) {
                        printf("fi_mr_r_bind error (%d)\n", ret);
                        return ret;
                }

		ret = fi_mr_enable(mr_w);
		if (ret) {
			printf("fi_mr_enable error (%d)\n", ret);
			return ret;
		}

		ret = fi_mr_enable(mr_r);
                if (ret) {
                        printf("fi_mr_enable error (%d)\n", ret);
                        return ret;
                }
...
static void cleanup(void)
{
	...
	if (mr_w) {
		ret = fi_close(&mr_w->fid);
		if (ret)
			printf("warning: error closing EP (%d)\n", ret);
	}

	if (mr_r) {
                ret = fi_close(&mr_r->fid);
                if (ret)
                        printf("warning: error closing EP (%d)\n", ret);
        }
...
/* Do a write into the client's write buffer.
 * When doing a write, if the provider is not ready to process messages (because
...
{
	char *msg = "Hello, server! This is the WRITE you have been waiting for!\0";
	int ret;

	fill_buffer(buf_w, msg);
	//(void) snprintf(buf_w, BUF_SIZE, "%s", msg);

	do {
		ret = fi_write(ep, buf_w, strlen(msg), desc_w, fi_addr, remote_addr_w,
			       key_w, NULL);
		if (ret && ret != -FI_EAGAIN) {
...
/* Do a read from the server's read buffer.
 * When doing a write, if the provider is not ready to process messages (because
 * of connection initialization for example), it may return -FI_EAGAIN. This does
 * not indicate an error, but rather that the application should try again later.
 * This is why we almost always wrap sends and receives in a do/while. Some providers
...
static int do_read(void)
{
	int ret;
	do {
                ret = fi_read(ep, buf_r, BUF_SIZE, desc_r, fi_addr, remote_addr_r,
                               key_r, NULL);
                if (ret && ret != -FI_EAGAIN) {
...
static int run(void)
{
	char *msg = "Hello, server! This is the READ you have been waiting for!\0";
	int ret;
